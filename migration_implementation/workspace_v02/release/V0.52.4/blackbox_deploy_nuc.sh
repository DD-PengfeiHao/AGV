#!/usr/bin/env bash
# V0.52.4 BlackBox-only deploy into delivery_gazebo_soft (no AGV motion tests)
set -eu
TMP="${1:-/tmp/v0524_deploy}"
...
docker exec "$C" bash -lc "grep -m1 'VERSION = \"0.52.4\"' $PY_DW/dashboard_node.py" || fail "VERSION 0.52.4 not in install path"
