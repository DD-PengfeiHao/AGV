...
echo "========== VERSION =========="
VER=$(docker exec "$C" curl -sf -m 8 "$BASE/api/version") || fail "version endpoint"
echo "$VER" | grep -q '0.52.4' || fail "expected version 0.52.4: $VER"
ok "version 0.52.4"

echo "========== LOGIN =========="
LOGIN=$(docker exec "$C" curl -sf -m 8 -X POST "$BASE/api/auth/login" -H "Content-Type: application/json" -d "{\"username\":\"$USER\",\"password\":\"$PASS\"}") || fail "login"
...
ST0=$(docker exec "$C" curl -sf -m 8 "$BASE/api/blackbox/status" -H "Authorization: Bearer $TOKEN") || fail "blackbox status"
echo "$ST0" | grep -q '"available": true' || fail "blackbox unavailable: $ST0"
ok "blackbox available"

echo "========== WARM BUFFER ${WARM_SEC}s =========="
sleep "$WARM_SEC"
ST1=$(docker exec "$C" curl -sf -m 8 "$BASE/api/blackbox/status" -H "Authorization: Bearer $TOKEN")
echo "$ST1" | head -c 400
echo
...
TR=$(docker exec "$C" curl -sf -m 8 -X POST "$BASE/api/blackbox/trigger" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d "{\"source\":\"nuc_verify\",\"key\":\"API\"}") || fail "trigger"
echo "$TR"
...
for i in $(seq 1 35); do
  sleep 1
  ST=$(docker exec "$C" curl -sf -m 8 "$BASE/api/blackbox/status" -H "Authorization: Bearer $TOKEN")
...
docker exec "$C" test -f "$MANIFEST" || fail "manifest missing: $MANIFEST"
docker exec "$C" python3 -c "import json; m=json.load(open('$MANIFEST')); assert m.get('record_id')=='$RID'; assert m.get('duration_sec',0)>=40; print('manifest ok', m.get('sources'))"
ok "manifest $MANIFEST"

echo "========== PRE/POST WINDOW FILES =========="
docker exec "$C" test -f "/home/ubuntu/Pengfei.Hao/blackbox/${RID:0:8}/$RID/snapshot/state.json"
docker exec "$C" test -f "/home/ubuntu/Pengfei.Hao/blackbox/${RID:0:8}/$RID/state/agv.jsonl"
docker exec "$C" wc -l "/home/ubuntu/Pengfei.Hao/blackbox/${RID:0:8}/$RID/state/agv.jsonl"
