...
class ArmBridge:
    ...
    def __init__(self, node: Any, robot_ip: str = "172.31.0.123") -> None:
        ...
        try:
            ...
            node.create_subscription(String, "/pick_place_state", self._on_state, 10, callback_group=cg)
            node.create_subscription(JointState, "/joint_states", self._on_joints, 10, callback_group=cg)
            self._unlock_client = node.create_client(Trigger, "/unlock_and_home", callback_group=cg)
            self._pickplace_client = node.create_client(Trigger, "/do_pick_place", callback_group=cg)
            self._gripper_client = node.create_client(SetBool, "/set_gripper", callback_group=cg)
            try:
    ...
    def call_gripper(self, open_command: bool, timeout: float = 5.0) -> Dict[str, Any]:
        ...
        with self._svc_lock:
            ...
            if not self._wait_future(future, timeout):
                return {"success": False, "message": "Timeout (5s)"}
            try:
                result = future.result()
            except Exception as exc:  # noqa: BLE001
                return {"success": False, "message": str(exc)}
