...
class ArmBridge:
    ...
    def __init__(
        ...
        self._svc_unlock = service_unlock

        self._svc_pick_place = service_pick_place

        self._svc_gripper = service_gripper

        self._topic_state = ros_state_topic

        self._topic_joints = ros_joint_topic
        ...
        try:
            ...
            node.create_subscription(String, self._topic_state, self._on_state, 10, callback_group=sub_cg)

            node.create_subscription(JointState, self._topic_joints, self._on_joints, 10, callback_group=sub_cg)

            self._unlock_client = node.create_client(Trigger, self._svc_unlock, callback_group=cli_cg)

            self._pickplace_client = node.create_client(Trigger, self._svc_pick_place, callback_group=cli_cg)

            self._gripper_client = node.create_client(SetBool, self._svc_gripper, callback_group=cli_cg)
    ...
    def _check_heartbeat(self) -> None:
        ...
        svc_online = self._service_ready(self._unlock_client)
        ...
        with self._lock:
            ...
            if self._is_busy and self._busy_since > 0:

                if (time.time() - self._busy_since) > self.BUSY_STALE_SEC:
    ...
    def _ros_cli_env(self) -> Dict[str, str]:

        env = os.environ.copy()

        env["ROS_DOMAIN_ID"] = str(self._ros_domain_id)
    ...
    def _parse_trigger_cli(self, text: str) -> Dict[str, Any]:
        ...
        return {"success": success, "message": msg or text.strip()}



    def _call_trigger_via_cli(self, service: str, timeout_sec: float) -> Dict[str, Any]:
        ...
        try:
            ...
            proc = subprocess.run(
                ...
                env=self._ros_cli_env(),

            )

        except Exception as exc:  # noqa: BLE001

            return {"success": False, "message": f"cli failed: {exc}", "via": "ros2_cli"}

        text = (proc.stdout or "") + "\n" + (proc.stderr or "")

        out = self._parse_trigger_cli(text)

        out["via"] = "ros2_cli"

        out["command"] = " ".join(cmd)

        if not out.get("success") and proc.returncode != 0:

            out["message"] = out.get("message") or text.strip() or f"exit {proc.returncode}"
    ...
    def _call_setbool_via_cli(self, service: str, data: bool, timeout_sec: float) -> Dict[str, Any]:

        yaml_body = "{data: true}" if data else "{data: false}"

        cmd = ["ros2", "service", "call", service, "std_srvs/srv/SetBool", yaml_body]
        ...
        if out.get("success"):
            ...
            self._gripper_state = "open" if data else "closed"
    ...
    def _call_trigger(self, client, name: str, timeout_sec: float) -> Dict[str, Any]:
        ...
        if client is None:
            ...
            return self._call_trigger_via_cli(name, timeout_sec)
        ...
        out = {"success": bool(result.success), "message": str(result.message or ""), "via": "ros2_client"}
    ...
    def set_cycles(self, cycles: int) -> Dict[str, Any]:
        ...
        if self._param_client is None:
            ...
            try:
                ...
                return {"success": True, "cycles": cycles, "message": f"cached locally ({exc})"}
    ...
    def call_unlock(self, timeout: float = 30.0) -> Dict[str, Any]:
        ...
        with self._svc_lock:
            ...
            out = self._call_trigger(self._unlock_client, self._svc_unlock, timeout)
    ...
    def call_pick_place(self, cycles: Optional[int] = None, timeout: float = 120.0) -> Dict[str, Any]:
        ...
        if cycles is not None:
            ...
            timeout = max(timeout, float(cycles) * 120.0)

        with self._svc_lock:

            st = self.get_status()

            if not st.get("online") and not self._service_ready(self._pickplace_client):

                return {"success": False, "message": "Arm offline (xArm stack not reachable)", "duration_s": 0.0}
            ...
            try:
                ...
                out = self._call_trigger(self._pickplace_client, self._svc_pick_place, timeout)
    ...
    def call_gripper(self, open_command: bool, timeout: float = 12.0) -> Dict[str, Any]:
        ...
        if self._gripper_client is None:
            ...
            return self._call_setbool_via_cli(self._svc_gripper, open_command, timeout)
        ...
        with self._svc_lock:
            ...
            return {"success": bool(result.success), "message": str(result.message or ""), "via": "ros2_client"}
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "service_unlock": self._svc_unlock,

                "service_pick_place": self._svc_pick_place,

                "service_gripper": self._svc_gripper,
