...
class RealAdapter(AgvApiAdapter):
    ...
    def diagnose(self) -> RobotState:
        ...
        mb_key = "manualBlock" if "manualBlock" in batch else "manual_block"
        if mb_key in batch:
            base.manual_block = _opt_bool(batch.get(mb_key))
        if "tracking_status" in batch:
            base.tracking_status = _opt_int(batch.get("tracking_status"))
